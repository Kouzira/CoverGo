...
# --- HÀM 6: HÀM CHAY CHINH (generate_report) - ĐÃ CẬP NHẬT ---
def generate_report(DATA_FOLDER='uploads'):
    # --- Cau hinh ---
    YOUR_API_KEY = "AIzaSyAd6srBMC1eEdGXqtPTT0uDrUePX4IQ3S4"
    ...
    if not os.path.exists(DATA_FOLDER):
        print(f"Loi: Khong tim thay thu muc CSV '{DATA_FOLDER}'")
        return
    ...
    all_charts_info = [] 
    csv_files = glob.glob(os.path.join(DATA_FOLDER, '*'))
    
    if not csv_files:
        print(f"Loi: Khong tim thay file CSV nao trong thu muc '{DATA_FOLDER}'.")
        return
